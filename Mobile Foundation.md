...
To accomplish ubiquity we developed a super dumbed down HTML 1.0 interface that would work on 90% of all small screen devices, remember pre-smart phone, and for the remaining devices serve up alternative markup that would display content correctly and consistent with the company's brand/look and feel.  How do we know when to serve up the correct markup? By analyzing the devices user agent which includes the device's screen size, operating system, and other pertinent information. Over time with all the customers that fidelity had, the company developed an exhaustive database of devices their customers used. With that information they could serve up markup that would work on the majority of their clients smallscreen devices.
